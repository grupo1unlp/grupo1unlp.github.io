...
function noLaborable() {
		var num = 0;
	$("div.simcal-day-has-events").each(function(i, obj) {
		var feriado = false;
		$(this).find(".simcal-event-title").each(function(o, obj1) {
			var titulo = $( this ).text().toLowerCase().replace(" ", "");
			if (titulo == "nolaborable") {
				feriado = true;
			}	
			if (feriado) {
				var descripcion = $(this).parent().next().children();
				var titulo_bien = descripcion.text();
				descripcion.text("");
				$(this).text(titulo_bien);
				$(this).parent().parent().find("p > .no-laborable").css("display", "inline");
			}
		});
...
function noLaborableGrid() {
	var num = 0;
	$("td.simcal-day-has-events").each(function(i, obj) {
		var feriado = false;
		$(this).find(".simcal-event-title").each(function(o, obj1) {
			var titulo = $( this ).text().toLowerCase().replace(" ", "");
			if (titulo == "nolaborable") {
		...
		if (feriado) {
			$(this).css({"border": "solid #61b0f7 2px"});
		}
